...
{
    public class Validar03 : brGenerico
    {
            ...
            try
            {
              ...
              (decimal.TryParse(lCabecera[50], out temp) ? temp : 0);
                ...
                if (lCabecera[0] == "210")
                {
                    ...
                    {
                        lRechazos.AddRange(await AgregarRechazo(lCabecera, "612", "SI EL TIPO DE OPERACION ES 1001 Y EL CAMPO 29 TIENE EL VALOR 1, SE DEBE ENVIAR LA LINEA DETRACCION.", txt));
                    }
                    ...
                    //}
                    if (lCabecera[3].Trim() == "0205" || lCabecera[3].Trim() == "0202")
                    {
                        lRechazos.AddRange(await AgregarRechazo(lCabecera, "621", "ESTE TIPO DE OPERACION(0205 - 0202) NO SE ADMITE EN BOLETAS .", txt));
                    }
                    ...
                    if (!string.IsNullOrWhiteSpace(lCabecera[6]) && DateTime.TryParseExact(lCabecera[6], formatos, null, System.Globalization.DateTimeStyles.None, out FEmision))
                    {
                        ...
                        //TimeSpan diferencia = DateTime.Now - FEmision;
                        if ((DateTime.Now.Date - FEmision.Date).TotalDays > 5 && UTilidades.Documentos5Dias(lCabecera[2]))
                        {
                    ...
                    {
                        lRechazos.AddRange(await AgregarRechazo(lCabecera, "535", "DEBE HABER LINEA CLIENTE EN LA BOLETA.", txt));
                    }
                    string lineaItem = Array.Find(lineas, x => x.ToUpper().Contains("BIEN") || x.ToUpper().Contains("SERVICIO"));
                    if (lineaItem == null)
                    {
                        lRechazos.AddRange(await AgregarRechazo(lCabecera, "534", "DEBE HABER AL MENOS 1 DETALLE EN LA BOLETA.", txt));
                    }
                    ...
                    if (parts[1].ToUpper().Trim() == "BIEN" || parts[1].ToUpper().Trim() == "SERVICIO")
                    {
                        ...
                        if (anticipo.Count > 0)
                        {
                            foreach (var item in anticipo)
                            {
                                ...
                                var anticipos = new brConsultar().BoletaAnticipo_Obtener(lCabecera[4], docCliente[2], docCliente[1], anticipo_[4].Trim(), anticipo_[5]);
                                ...
                                if (anticipo_[3] != "03")
                                {
                                    lRechazos.AddRange(await AgregarRechazo(lCabecera, "639", "EL CODIGO DEL TIPO DE COMPROBANTE QUE REALIZA ANTICIPO TIENE QUE SER 03.", txt));
                                }
                        ...
                        if (igv < 0.01M && igv > 0)
                        {
                            lRechazos.AddRange(await AgregarRechazo(lCabecera, "582", $"EL IGV DEL DETALLE NO PUEDE SER MENOR A 0.01. - linea:{parts[0]}", txt));
                        }
                        ...
                        if (OCTOTAL > 0)
                        {
                            ...
                            else
                            {
                                if (otrosCargos.Count > 0)
                                {
                                    ...
                                    if (codigo == "49")
                                    {
                                        if (IMPORTETOTAL < ((BASEIMPONIBLE + IGV - SumDetalleGratuitos)))
                                        {
                                            lRechazos.AddRange(await AgregarRechazo(lCabecera, "635", "EL IMPORTE TOTAL NO PUEDE SER MENOR A LA SUMA DE TOTAL OTROS CARGOS + BASE IMPONIBLE.", txt));
                                        }
                                    }
                        ...
                        if (SumDetalleTotal > 0)
                        {
                            if (descuentoGlobal.Count > 0)
                            {
                                ...
                                if (codigo == "02")
                                {
                                    var nuevaBI = BASEIMPONIBLE - SumDetalleGratuitos;
                                    var nuevoIGV = nuevaBI * configuracion.CSuc_PorcentajeIGV;
                                    if (Math.Abs((nuevaBI + nuevoIGV + ISCTOTAL + OCTOTAL + OTTOTAL) - IMPORTETOTAL) > tolerancia)
                                    {
                                        lRechazos.AddRange(await AgregarRechazo(lCabecera, "635", "ERROR EN EL CÁLCULO DEL IMPORTE TOTAL EN LA CABECERA.", txt));
                                    }
                                }
                        ...
                        if (descuentoGlobal.Count == 0 && otrosCargos.Count == 0)
                        {
                            ...
                            else
                            {
                                if (columnas.Count > 3)
                                {
                                    if (Math.Abs((SumDetalleBI - SumDetalleDESC) - (BASEIMPONIBLE - DESCUENTO)) > tolerancia && anticipo.Count == 0)
                                    {
                                        lRechazos.AddRange(await AgregarRechazo(lCabecera, "376", "LA SUMA DE LA BASE IMPONIBLE DEL DETALLE NO CUADRA CON LA BASE IMPONIBLE TOTAL DE LA CABECERA.", txt));
                                    }
                                }
                        ...
                        if (descuentoGlobal.Count == 0 && otrosCargos.Count == 0)
                        {
                            if (anticipo.Count > 0)
                            {
                                ...
                                foreach (var item in anticipo)
                                {
                                    var anti = item.Split('|');
                                    var docCliente = clienteLine.Split('|');
                                    var anticipos = new brConsultar().BoletaAnticipo_Obtener(lCabecera[4], docCliente[2], docCliente[1], anti[4], anti[5]);
                                    decimal montoPendienteAnticipo = new brConsultar().ObtenerMontoPendienteAnticipo(lCabecera[4], "03", anti[4], anti[5]);
                                    sumAnti += montoPendienteAnticipo;
                                    sumIGV += anticipos == null ? 0 : anticipos.IGV;
                                }

                                if (SummontoAnticipado != sumAnti)
                                {
                                    if (Math.Abs(Math.Round(IGV + sumIGV, configuracion.CSuc_CantidadDecimal) - Math.Round(SumDetalleIGV, configuracion.CEmi_CantidadDecimalDetalle)) > tolerancia)
                                    {
                        ...
                        if (descuentoGlobal.Count > 0)
                        {
                            ...
                            var nuevaBI = SumDetalleBI - montoDesc;
                            ...
                            {
                                if (Math.Abs(SumDetalleIGV - IGV) > tolerancia)
                                {
                                    lRechazos.AddRange(await AgregarRechazo(lCabecera, "322", "LA SUMA DEL IGV DEL DETALLE NO CUADRA CON EL IGV TOTAL DE LA CABECERA.", txt));
                                }
                            }
                            ...
                            {
                                if (variables[3].Trim() == "02")
                                {
                                    var newIGV = (SumDetalleBI - montoDesc) * configuracion.CSuc_PorcentajeIGV;
                                    if (Math.Abs(IGV_Calculado - newIGV) > tolerancia)
                                    {
                                        lRechazos.AddRange(await AgregarRechazo(lCabecera, "322", "LA SUMA DEL IGV DEL DETALLE NO CUADRA CON EL IGV TOTAL DE LA CABECERA.", txt));
                                    }
                                }
                        ...
                        if (motivo.Length > 3 && motivo[3].Trim() != "02")
                        {
                            ...
                            if (motivo[3].Trim() == "03")
                            {
                                ...
                                {
                                    if (Math.Abs(SumDetalleBI - BASEIMPONIBLE) > tolerancia)
                                    {
                                        lRechazos.AddRange(await AgregarRechazo(lCabecera, "376", "LA SUMA DE LA BASE IMPONIBLE DEL DETALLE NO CUADRA CON LA BASE IMPONIBLE TOTAL DE LA CABECERA", txt));
                                    }
                                }
                ...
                if (descuentoGlobal.Count > 0)
                {
                    foreach (var item in descuentoGlobal)
                    {
                        ...
                        if (parts[3].Trim() == "02")
                        {
                            ...
                            {
                                var igvnew = (SumDetalleBIconDescGlobal - descuento) * configuracion.CSuc_PorcentajeIGV;
                                if (Math.Abs(((SumDetalleBIconDescGlobal - descuento) + igvnew) - IMPORTETOTAL) > tolerancia && !exportacion)
                                {
                                    lRechazos.AddRange(await AgregarRechazo(lCabecera, "392", "LA SUMA DEL IMPORTE TOTAL DE LOS DETALLES MENOS EL MONTO DEL DESCUENTO GLOBAL, DEBE SER IGUAL AL IMPORTE TOTAL DE LA CABECERA", txt));
                                }
                            }
                ...
                if (anticipo.Count > 0)
                {
                    ...
                    foreach (var item in anticipo)
                    {
                        var parts = item.Split('|');
                        var docCliente = clienteLine.Split('|');
                        var anticipos = new brConsultar().BoletaAnticipo_Obtener(lCabecera[4], docCliente[2], docCliente[1], parts[4], parts[5]);
                        var montoPendienteAnticipo = new brConsultar().ObtenerMontoPendienteAnticipo(lCabecera[4], "03", parts[4], parts[5]);
                        ...
                        //if (anticipos != null)
                        //{
                        //    if (fechaDocumentoAnticipado != Convert.ToDateTime(anticipos.FechaEmision))
                        //    {
                        ...
                        //    }
                        //}
                        if (parts[3].Trim() != lCabecera[2].Trim())
                        {
                        ...
                        if (!string.IsNullOrEmpty(parts[4].Trim()))
                        {
                            ...
                            if (!Regex.IsMatch(campo, @"^B[A-Z0-9]{3}$"))
                            {
                ...
                if (detraccion.Count > 0)
                {
                    foreach (var item in detraccion)
                    {
                        ...
                        if (string.IsNullOrEmpty(parts[1].Trim()) || parts[1].Trim() != "037")
                        {
                        ...
                        var tipoCambio = new brConsultar().ObtenerTipoCambio(Femision.ToString("yyyy-MM-dd"), lCabecera[8].Trim());
                ...
                if (clienteLine != null)
                {
                    ...
                    if (!rucCache.TryGetValue(parts[2], out int existeRuc))
                    {
                        ...
                        if (parts[0].ToUpper().Trim() == "CLIENTE")
                        {
                            ...
                            if (IMPORTETOTAL > 700 &&
                                (string.IsNullOrWhiteSpace(parts[2]) || string.IsNullOrWhiteSpace(parts[3]) || string.IsNullOrWhiteSpace(parts[7])))
                            {
                                lRechazos.AddRange(await AgregarRechazo(lCabecera, "307", "BOLETA CON MONTO TOTAL MAYOR A 700 SIN INFORMACION DEL CLIENTE COMPLETA. CAMPOS OBLIGATORIOS: DOCUMENTO DEL CLIENTE, NOMBRES O RAZON SOCIAL Y DIRECCION.", txt));
                            }
                ...
                if (detalleFP.Count > 0)
                {
                    lRechazos.AddRange(await AgregarRechazo(lCabecera, "627", "LA FORMA DE PAGO ES SOLO PARA FACTURAS Y NOTAS DE CREDITO DE FACTURAS.", txt));
                }
            }
            catch (Exception ex)
            {
                await LogAsync($"Validar03 - {txt[5]}", ex);
