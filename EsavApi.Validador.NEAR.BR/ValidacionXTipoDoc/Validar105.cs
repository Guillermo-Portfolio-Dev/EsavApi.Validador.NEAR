            ...
            try
            {
              ...
              (decimal.TryParse(lCabecera[50], out temp) ? temp : 0);
                ...
                if (lCabecera[0] == "210")
                {
                    ...
                    {
                        lRechazos.AddRange(await AgregarRechazo(lCabecera, "535", "DEBE HABER LINEA CLIENTE EN EL VALE DE CREDITO.", txt));
                    }
                    string lineaItem = Array.Find(lineas, x => x.ToUpper().Contains("BIEN") || x.ToUpper().Contains("SERVICIO"));
                    if (lineaItem == null)
                    {
                        lRechazos.AddRange(await AgregarRechazo(lCabecera, "534", "DEBE HABER AL MENOS 1 DETALLE EN EL VALE DE CREDITO.", txt));
                    }
